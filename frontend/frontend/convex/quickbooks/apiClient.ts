...
export const getCompanyInfo = internalQuery({
  ...
  handler: async (ctx, args) => {
    // Get valid token
    const tokenData = await ctx.runMutation(internal.quickbooks.tokenManagerV2.getValidToken, {
      companyId: args.companyId,
    });
    
    // Get company data
    const company = await ctx.db.get(args.companyId);
    if (!company || !company.realmId) {
 ...
 */
export const getInvoices = mutation({
  args: {
    companyId: v.id("companies"),
  ...
  handler: async (ctx, args) => {
    ...
    // Execute query
    const invoices = await executeQuery<Invoice>(tokenData.accessToken, baseUrl, query);
 ...
 */
export const getBills = mutation({
  args: {
    companyId: v.id("companies"),
    startDate: v.optional(v.string()),
    endDate: v.optional(v.string()),
  ...
  handler: async (ctx, args) => {
    ...
    // Execute query
    const bills = await executeQuery<Bill>(tokenData.accessToken, baseUrl, query);
 ...
 */
export const getAccounts = mutation({
  args: {
    companyId: v.id("companies"),
  ...
  handler: async (ctx, args) => {
    ...
    // Execute query
    const accounts = await executeQuery<Account>(tokenData.accessToken, baseUrl, query);
 ...
 * Get customers
 */
export const getCustomers = mutation({
  args: {
    companyId: v.id("companies"),
    onlyActive: v.optional(v.boolean()),
    limit: v.optional(v.number()),
  ...
  handler: async (ctx, args) => {
    ...
    // Execute query
    const customers = await executeQuery<Customer>(tokenData.accessToken, baseUrl, query);
 ...
 */
export const getCashFlowData = mutation({
  args: {
    companyId: v.id("companies"),
  ...
  handler: async (ctx, args) => {
    ...
    const [invoices, bills, accounts] = await Promise.all([
      ctx.runMutation(api.quickbooks.apiClient.getInvoices, {
        companyId: args.companyId,
        startDate: startDate.toISOString().split('T')[0],
        status: "All",
      }),
      ctx.runMutation(api.quickbooks.apiClient.getBills, {
        companyId: args.companyId,
        startDate: startDate.toISOString().split('T')[0],
        status: "All",
      }),
      ctx.runMutation(api.quickbooks.apiClient.getAccounts, {
        companyId: args.companyId,
        accountType: "Bank",
        onlyActive: true,
      }),
    ...
    const bankAccounts = accounts.accounts.filter(acc => 
      acc.type === "Bank" && acc.balance !== undefined
    );
    const currentCash = bankAccounts.reduce((sum, acc) => sum + (acc.balance || 0), 0);
