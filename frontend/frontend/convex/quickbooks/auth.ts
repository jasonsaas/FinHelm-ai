...
export const generateAuthUrl = mutation({
  ...
  handler: async (ctx, args) => {
    ...
    // Store PKCE challenge in database for later verification
    await ctx.db.patch(args.companyId, {
      metadata: {
        pkceVerifier: pkce.codeVerifier,
        pkceState: pkce.state,
        pkceExpiry: Date.now() + 600000, // 10 minutes expiry
      },
    });
...
export const exchangeCodeForToken = mutation({
  ...
  handler: async (ctx, args) => {
    ...
    if (company.metadata?.pkceState !== args.state) {
      throw new QuickBooksAuthError(
    ...
    // Check PKCE expiry
    if (!company.metadata?.pkceExpiry || company.metadata.pkceExpiry < Date.now()) {
      throw new QuickBooksAuthError(
        "PKCE challenge expired",
        "PKCE_EXPIRED"
      );
    }

    const codeVerifier = company.metadata.pkceVerifier;
    if (!codeVerifier) {
      throw new QuickBooksAuthError(
    ...
    try {
      ...
      await ctx.db.patch(args.companyId, {
        ...
        metadata: {
          ...
          pkceVerifier: undefined, // Clear PKCE data
          pkceState: undefined,
          pkceExpiry: undefined,
        },
      });
...
export const refreshAccessToken = mutation({
  ...
  handler: async (ctx, args) => {
    ...
    const refreshTokenExpiry = company.metadata?.refreshTokenExpiry;
...
export const checkTokenValidity = query({
  ...
  handler: async (ctx, args) => {
    ...
    const refreshTokenValid = refreshTokenExpiry > now;
