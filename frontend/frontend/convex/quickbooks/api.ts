...
class RateLimiter {
  ...
  async waitForRateLimit(companyId: string): Promise<void> {
    ...
    if (!check.allowed && check.retryAfter && this.config.retryAfterRateLimit) {
      ...
      await new Promise(resolve => setTimeout(resolve, Math.min(check.retryAfter, 60000)));
 ...
 */
export const createClient = internalQuery({
  args: {
  ...
  handler: async (ctx, args) => {
    ...
    const tokens = await ctx.runQuery(internal.quickbooks.tokenManager.getTokens, {
    ...
    await ctx.runMutation(internal.quickbooks.tokenManager.ensureValidToken, {
      companyId: args.companyId,
    });
...
export const syncInvoices = mutation({
  ...
  handler: async (ctx, args) => {
    ...
    const client = await ctx.runQuery(internal.quickbooks.api.createClient, {
      companyId: args.companyId,
    });
