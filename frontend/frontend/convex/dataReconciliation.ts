...
export const reconcileChartOfAccounts = action({
  ...
  handler: async (ctx, args) => {
    ...
    try {
      // Parse CSV data
      const csvRecords = await parseCSVData(ctx, {
      ...
      const existingAccounts = await ctx.runQuery("dataReconciliation:getAccountsByCompany", {
      ...
      if (!args.options?.dryRun) {
        await ctx.runMutation("dataReconciliation:storeReconciliationResult", {
...
export const reconcileTransactions = action({
  ...
  handler: async (ctx, args) => {
    ...
    try {
      ...
      const existingTransactions = await ctx.runQuery("dataReconciliation:getTransactionsByCompany", {
        companyId: args.companyId
      });
  ...
  handler: async (ctx, args) => {
    ...
    for (const dataset of args.datasets) {
      ...
      if (dataset.type === "accounts") {
        result = await reconcileChartOfAccounts(ctx, {
          companyId: args.companyId,
          csvData: dataset.csvData,
          options: dataset.options
        });
      } else {
        result = await reconcileTransactions(ctx, {
          companyId: args.companyId,
          csvData: dataset.csvData,
          options: dataset.options
        });
...
export const storeReconciliationResult = mutation({
  ...
  handler: async (ctx, args) => {
    ...
    return await ctx.db.insert("syncLogs", {
      companyId: result.matches[0]?.sourceRecord?.companyId || "unknown",
      syncType: "reconciliation",
      status: "completed",
      recordsProcessed: result.totalRecords,
      recordsSuccessful: result.matchedRecords,
      recordsFailed: result.unmatchedRecords,
      errorDetails: result.unmatchedRecords > 0 ? `${result.unmatchedRecords} unmatched records` : undefined,
      metadata: {
...
export const getReconciliationStats = query({
  ...
  handler: async (ctx, args) => {
    ...
    const totalRecordsMatched = logs.reduce((sum, log) => sum + (log.recordsSuccessful || 0), 0);
    const totalConfidence = logs.reduce((sum, log) => sum + (log.metadata?.averageConfidence || 0), 0);
    const totalProcessingTime = logs.reduce((sum, log) => sum + (log.metadata?.processingTimeMs || 0), 0);
