...
export const getTokens: any = internalQuery({
  ...
  handler: async (ctx, args) => {
    ...
    try {
      // Decrypt tokens
      // In production, you would decrypt the token
...
export const autoRefreshToken: any = internalMutation({
  ...
  handler: async (ctx, args) => {
    ...
    // Check if refresh token is still valid (approximation)
    const refreshTokenExpiry = company.tokenExpiry ? company.tokenExpiry + (100 * 24 * 60 * 60 * 1000) : 0;
    if (refreshTokenExpiry < now && company.refreshToken) {
 ...
 */
export const scheduleTokenRefresh: any = mutation({
  args: {
    companyId: v.id("companies"),
  },
  handler: async (ctx, args) => {
 ...
 * Clear stored tokens
 */
export const clearTokens: any = mutation({
  args: {
    companyId: v.id("companies"),
  },
  handler: async (ctx, args) => {
 ...
 */
export const getTokenStatus: any = query({
  args: {
    companyId: v.id("companies"),
  },
  handler: async (ctx, args) => {
 ...
 */
export const ensureValidToken: any = internalMutation({
  args: {
    companyId: v.id("companies"),
  },
  handler: async (ctx, args) => {
