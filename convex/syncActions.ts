...
/**
 * Sync ERP data with fuzzy matching and reconciliation
 */
export const syncERPData = action({
  ...
  handler: async (ctx, args) => {
    ...
    try {
      ...
      console.log(`ERP sync completed for org ${args.organizationId}: ${JSON.stringify(results)}`);
...
async function syncAccountsWithReconciliation(
  ...
  const existingAccounts = await ctx.runQuery(api.accountActions.getAccountHierarchy, {
    organizationId: args.organizationId,
    includeInactive: true,
  });

  const flatExistingAccounts = flattenAccountHierarchy(existingAccounts);
      ...
      if (match && args.options.autoApplyHighConfidenceMatches && match.score > 0.9) {
        ...
        if (existingAccount) {
          await ctx.runMutation(api.accountActions.upsertAccount, {
            organizationId: args.organizationId,
            erpConnectionId: args.erpConnectionId,
            externalId: sourceAccount.externalId || sourceAccount.code,
            code: sourceAccount.code,
            name: sourceAccount.name,
            type: mapAccountType(sourceAccount.type),
            category: sourceAccount.category,
            subType: sourceAccount.subType,
            parentCode: sourceAccount.parentCode,
...
async function syncTransactionsWithValidation(
  ...
  // Process transactions (simplified for now)
  for (const txn of args.sourceTransactions) {
    try {
      // Validate required fields
      if (!txn.accountCode || !txn.amount || !txn.transactionDate) {
        results.errors.push(`Transaction ${txn.id} missing required fields`);
        continue;
      }

      // Find matching account
      const account = await ctx.runQuery(api.accountActions.getAccount, {
        organizationId: args.organizationId,
        accountCode: txn.accountCode,
      });

      if (!account) {
        results.errors.push(`Account ${txn.accountCode} not found for transaction ${txn.id}`);
        continue;
      }
      ...
      const transactionData = {
        ...
        transactionDate: new Date(txn.transactionDate).getTime(),
        postingDate: txn.postingDate ? new Date(txn.postingDate).getTime() : undefined,
        dueDate: txn.dueDate ? new Date(txn.dueDate).getTime() : undefined,
